{% load i18n menu_tags cache %}{% spaceless %}
    {% for child in children %}
        <li class="nav-item {% if child.ancestor %}ancestor{% endif %}{% if child.children %} dropdown{% endif %}">
            {% if child.children %}
            <a class="nav-link dropdown-toggle{% if child.selected %} active{% endif %}" role="button" data-bs-toggle="dropdown" data-request="{{ request.path }}" x href="{{ child.get_absolute_url }}" id="menu-{{ child.id|safe }}">
                {{ child.get_menu_title }}
            </a>
                <div class="dropdown-menu" aria-labelledby="menu-{{ child.ancestor.id|safe }}">
                    {% show_menu from_level to_level extra_inactive extra_active "bootstrap5/dropdown.html" "" "" child %}
                </div>
            {% else %}
                <a class="nav-link{% if child.selected %} active{% endif %}" data-request="{{ request.path }}"  href="{{ child.get_absolute_url }}">
                    <span>{{ child.get_menu_title }}</span>
                </a>
            {% endif %}
        </li>
    {% endfor %}
{% endspaceless %}
